...
- **Disposition:** T69 acceptance (line 103) reworded to remove the route and point here (**Reflected**). T74's reference left explicitly for Phase 16 (**Explicitly Deferred**). The backfill *code* is statically reviewed + `pnpm test`/`build` green. Runtime migration verification is **Explicitly Deferred**: resolution = add a `fake-indexeddb`-based real-Dexie migration test in Phase 16 (no debug route — adding a route repeats the phantom-reference problem). Rationale: `version(3).upgrade()` correctness warrants automated coverage; manual one-time checks have no rerun path.
...
| 3 | T74 (Phase 16) acceptance also cites `debug-indexeddb` (line 183) | Left **Explicitly Deferred** — Phase 16 task, not edited from the Phase 15 branch; reconcile to remove `debug-indexeddb` reference when Phase 16 is planned/executed | EXECUTION_PLAN T74 acceptance wording | **Explicitly Deferred** |
...
| 8 | T72 "offer to recreate" — SCHEMA.md says *"If no Node with a required `systemRole` exists, the system offers to recreate it."* Phase 15 is data-layer only, no UI. How should "offer" be interpreted? | Phase 15 = **silent idempotent startup handling**. `ensureSystemNodes()` runs on every app mount and handles two cases: (A) missing system node → created with full `nodeSchema` defaults; (B) lifecycle-drifted system node (`deletedAt` or `archivedAt` non-null) → both fields normalized to `null` in a single `write()`. No toast, dialog, or user confirmation in either case. The visual "offer" UX (sidebar prompt, confirmation) is a Phase 17+ concern. | None — EXECUTION_PLAN T72 "offer to recreate" wording is intentionally data-layer-agnostic; this entry is the Phase 15 interpretation record. | **Reflected** |
