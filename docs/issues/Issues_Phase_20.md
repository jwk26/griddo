...
| ISSUE-20-04 | Deferred/Behavior | System Nodes (`systemRole: "inbox"` and `systemRole: "archive_view"`) currently inherit the standard NodeCard aging filter. They are portal/infrastructure Nodes and should not visually age like user-created Nodes. Future fix should exclude non-null `systemRole` Nodes from aging visual treatment while preserving normal aging for regular Nodes and Scratch/Bit items. | Deferred — bug/behavior follow-up, not brainstorming |
