...
| Exact scope | Modify only the approved placement hook/test, Explorer/test, global CSS, inbox-triage copy/test, and the `P28-07`-approved Workspace/direct mounted test owners; the Workspace delta is limited to authoritative source/candidate projection, no-write invalidation wiring, once-only announcement, and the existing Breakdown grip/staged candidate/Staging heading focus fallback proof; create `docs/verification/inbox-triage/task-154.md`; update only this ledger state and the actual measured Task 154 workflow-pilot row |
...
| Issues / deviations | `P28-07` — Closed by explicit user approval. `P28-08` — Awaiting User Decision after High-risk review found that the new Workspace source projection cannot distinguish the initial empty `useScratchBreakdowns` snapshot from an authoritative empty snapshot without a readiness signal from that existing hook owner. Two other review findings remain inside the already approved paths and have not entered repair cycle 3. Task 153 reliability semantics and closed `P28-06` remain unchanged. |
| Canonical impact | `Reflected` for closed `P28-07`; `Tagged` for pending `P28-08`. The accepted `DP-VQ09` product meaning is unchanged, and no new owner has been added to the current Task 154 contract. |
| Next legal action | Stop for explicit `P28-08` disposition. If approved, reflect only the minimum readiness owner/test expansion before that product write, then use repair cycle 3 for all concrete review findings and rerun invalidated gates. Keep Task 154 `[ ]` and do not start another task. |
...
| `P28-08` | `Awaiting User Decision` | Post-focused-gate High-risk review found a concrete direct-placement race: `useScratchBreakdowns` returns `breakdowns: []` both before its first live-query snapshot and for an authoritative empty result, while Workspace currently invalidates on absence. A valid mounted source can therefore be falsely closed before the added Workspace hook instance becomes ready. Timing heuristics, hidden state, direct DataStore subscription, or a second DOM/event channel are prohibited and cannot prove authoritative absence. The same review also found two in-scope repairs: edited Result Title reliability copy/source-truth separation, and target/path invalidation once-only source focus/announcement. Evidence and current checks are recorded in `docs/verification/inbox-triage/task-154.md`. | Minimum recommended approval: add only `src/hooks/use-scratch-breakdowns.ts` and `src/hooks/use-scratch-breakdowns.test.tsx` as Task 154 owners to expose and directly test a read-only explicit readiness projection already derived from that hook's current snapshot. No query, editor, command, DataStore, schema, persistence, or other Breakdown behavior changes. The existing approved paths then consume the readiness signal and repair the other two findings. Rejecting this owner expansion requires an explicit acceptance amendment because the current API cannot distinguish loading from authoritative disappearance. | `Tagged`; no contract write or affected product write is authorized before user disposition. Accepted `DP-VQ09`, its recipe/design authority, and Task 153 semantics remain unchanged. |
