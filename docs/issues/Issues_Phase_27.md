...
> State: Tasks 136–148 accepted; P27-12/P27-13 bounded smoke repair in progress
...
| `P27-12` | In Progress | In the canonical `1440×900` GridDO light route with default `DESC` Breakdown sort, confirmed Add calls `scrollIntoView({ block: "start" })` for the new top row, which can move the complete Selected Scratch Context out of view. | On 2026-08-24 the user approved a bounded accepted-Task-136 smoke repair limited to Breakdown component/test plus plan, ledger, and repair evidence. Keep the new row visible with the minimum scroll that does not unnecessarily move Context; preserve Add focus, sort order, command/reconciliation, `Added.` identity/lifetime, and existing ASC row visibility. | `Reflected` — update accepted Task 136's Add scroll acceptance without reopening its `[x]` marker; implementation and evidence remain pending in this run. |
| `P27-13` | In Progress | A staged source grip is correctly rendered as a native disabled button, but its unconditional `cursor-grab active:cursor-grabbing` classes still advertise draggable feedback on pointer hover. | On 2026-08-24 the user approved a bounded accepted-Task-145 smoke repair in the same exact files. Disabled grips must expose a non-draggable cursor and no grab/grabbing cursor while active grips retain their existing feedback and Stage/DnD semantics. | `Reflected` — update accepted Task 145's staged-grip feedback acceptance without reopening its `[x]` marker; implementation and evidence remain pending in this run. |
...
| State | `In Progress`; Tasks 136–148 remain accepted and marked `[x]` |
...
| Next legal action | Add focused failing regressions, implement the bounded repairs, run skill-owned verification, commit a clean checkpoint, and stop at `P27-12/P27-13 Implemented; Awaiting Manual Smoke and Acceptance` |
