...
| `P27-08` | Awaiting User Decision | The approved `P27-07` projection owner resolves Stage/Unstage operation visibility, but two accepted Task 147 states still have no production signal. `cleanupConfirmedCandidateOrphan` and its reconciliation method have no non-test caller, so no mounted owner can observe an authoritative cleanup result or audit identity; separately, `useTriageDnd` closes or invalidates drag/placement state internally without exposing an invalidated-placement reason. Inferring either state from candidate disappearance or generic placement closure would violate the accepted proof and lifetime contract. | The resumed run stopped after the authorized canonical/durable-start commit and before every production/test/style/evidence write. A new user-approved repair boundary must identify the authoritative confirmed-orphan proof/command consumer and expose explicit invalidated-placement completion to the mounted Task 147 projection owner; Task 147 cannot substitute fixtures or disappearance inference. | `Tagged` — Task 146 hook/DnD/integrity owners or another already-authoritative producer must be explicitly approved and canonically named before Task 147 implementation resumes. |
...
| State | `Awaiting User Decision`; durable expanded start is committed, but `P27-08` stopped the run before production/test writes; Task 147 remains `[ ]`, and Task 148 remains `[ ]` and untouched |
...
| Additional drift | `P27-08`: no production caller emits authoritative confirmed-orphan cleanup results, and no explicit invalidated-placement reason reaches the mounted projection owner. No disappearance or generic close inference is permitted. |
| Next legal action | User disposition of `P27-08`: approve and canonically name the minimum authoritative proof/command and invalidated-placement signal owners, or revise the affected acceptance. Until then, do not start Task 147 production work or Task 148. |
