...
| `P27-07` | Approved / In Progress | Task 147 must render live Stage/Unstage pending, unknown, reconciling, and terminal status over Tasks 145–146, but its original production write set excluded the only mounted owner of those projections. `TriageWorkspaceContent` owns the command-bearing `useStagedCandidates` instance and passes its callbacks into `useTriageDnd`; the separately mounted `StagingZone` and `BreakdownPanel` hook instances never receive those operation transitions, and terminal outcomes are consumed inside `useDnd`. Component-only props or test fixtures would therefore not prove the canonical UI, focus, or lifetime behavior. | On 2026-08-21 the user confirmed checkpoint `1e296268c2b9d94c9393f42f25c08af6c661d18c` and approved the minimum Task 147 expansion to `src/components/triage/triage-workspace.tsx` and `.test.tsx` so the existing mounted owner can project its authoritative state into Staging/Breakdown. Task 146 hook/DnD/integrity semantics remain unchanged. | `Reflected` in `docs/EXECUTION_PLAN.md` Task 147 exact files/actions; implementation remains Task 147-only. |
| `P27-08` | Approved / In Progress | The approved `P27-07` projection owner resolves Stage/Unstage operation visibility, but two accepted Task 147 states needed explicit disposition. Confirmed-orphan cleanup has no authoritative proof producer or production caller; invalidated drag and placement already have explicit signals but Task 147 had not named them. Inferring either state from candidate disappearance or generic placement closure would violate the accepted proof and lifetime contract. | On 2026-08-21 the user approved `activeDragItem.integrity === "invalidated"` and existing `onPendingPlacementInvalidated(dropId)` as the only invalidation authorities projected by `triage-workspace.tsx`; no `use-dnd` or Hierarchy Explorer change is permitted. Confirmed-orphan copy/render remains in the headless state matrix, while production reachability and browser acceptance are deferred to a future remote-authority lifecycle; no proof, caller, injector, tombstone, or repository authority may be invented. | `Reflected` in `docs/EXECUTION_PLAN.md` Task 147 files/actions and verification boundary; the decision receipt remains unchanged. |
| `P27-09` | Awaiting User Decision | Task 147 remote-arrival counts must exclude initial hydration and Scratch-switch loads while still detecting a later remote arrival into an authoritatively empty subsection. `useStagedCandidates` currently returns the same empty projection before its first live-query snapshot and after a ready empty snapshot, and exposes no snapshot readiness. The approved Workspace owner therefore cannot distinguish those lifetimes without timing or first-nonempty inference. | The resumed run stopped after the authorized `P27-08` canonical/durable-start commit and before every production/test/style/evidence write. The minimum compatible repair is a read-only readiness projection from the existing `useStagedCandidates` snapshot owner plus its focused test; it must not change candidate truth, integrity classification, commands, DnD, or repository semantics. | `Tagged` — `src/hooks/use-staged-candidates.ts` and `.test.tsx` require explicit Task 147 write approval for readiness metadata before remote-arrival implementation can start. |
...
| State | `Awaiting User Decision`; `P27-08` is durably reflected, but `P27-09` stopped the run before production/test writes; Task 147 remains `[ ]`, and Task 148 remains `[ ]` and untouched |
...
| Additional drift | `P27-09`: the mounted candidate projection has no authoritative ready/loading distinction, so initial/Scratch-load exclusion cannot coexist with detecting later remote arrivals into a ready empty subsection without forbidden inference. |
| Next legal action | User disposition of `P27-09`: approve the minimum read-only readiness field and focused test in `src/hooks/use-staged-candidates.ts` and `.test.tsx`, or revise the affected remote-arrival acceptance. Until then, do not start Task 147 production work or Task 148. |
