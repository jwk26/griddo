...
> State: Tasks 136–148 accepted; Phase 27 smoke test pending separate approval
...
| `P27-11` | In Progress | Accepted Task 141 left component-level `liveQuery` and three direct `getDataStore()` reads in `src/components/triage/triage-workspace.tsx`, violating the blocking DataStore facade and hook API boundaries in `docs/PLANNING_STANDARD.md`. | On 2026-08-23 the user approved the minimum conformance repair: create `src/hooks/use-external-scratch-removal-data.ts` and `.test.tsx`, modify only Workspace and its test, and add plan/ledger/evidence records. Preserve exact Task 141 behavior, copy, DOM, style, timing, focus, lifecycle semantics, ordering, normalization, and race guards; do not modify DataStore, stores, repositories, schema, Tasks 142–148 behavior/evidence, `P27-06`, or `P27-08`. | `Reflected` in accepted Task 141's files/actions in `docs/EXECUTION_PLAN.md`; repair acceptance remains separately user-owned and this issue must not become `Closed` in the implementation lifecycle. |
